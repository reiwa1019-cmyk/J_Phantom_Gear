...
def check_password():
    ...
    if st.button("ログイン"):
        # secretsからパスワードを取得（設定がない場合のデフォルト値も用意）
...
def execute_transaction(tx_type, date_val, code_val, qty_val, price_val, is_bonus=False):
    if not IS_ADMIN: return # 閲覧者は実行不可
